...
void prepare( int dim )
{
    initboole( dim );
    initagldim( dim );
    bdg  =  monomialBasis( dmin  , dmax ,  4  );
    int r = orbitBasic( mkaglGroup( 4 ) , & bdg );
    printf("\norbit : %d\n", r);

  
    vector v;
 
    linear = calloc( bdg.size  , sizeof(vector ) );
    int k = 1;
    for( v = 0; v < bdg.size ; v++ ) {
	    boole f = vectortoboole( v, & bdg);
...
int main(int argc, char*argv[] )
{
    ...
    int orbsize;
    int ctrl = 0;
    base = monomialBasis( 2, 4,  4 );
    while   ( ( g = loadaglmapsize( src, &grp, &gsize ) ) ){
	    pTT( stdout, g );
	    ...
	    if ( grp ) {
		  initBrowse( &base );
		  aglVectorGroup  lda =  aglVectorGroupAction( grp, &base );
		  vector v;
		  ctrl = 0;
		  for(v = 0; v < base.size; v++ )
			  if ( base.table[v] == 0 ) {
		  		orbsize = browse( v , lda  );
				boole f = vectortoboole( v, &base );
				panfvalue( stdout, orbsize, f );
				free(f);
				count++;
				ctrl +=  orbsize;
		   ...
		   aglVectorGroupFree( lda );
		   free( base.table );
	    	   assert( ctrl ==  base.size );
	    } else count = base.size;
	    printf("\norbit=%d\n", count );
	    total += count;
	    free( g );
	    } 
